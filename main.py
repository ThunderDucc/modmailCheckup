...
async def help(ctx):
    ...
    emb.add_field(name="How does it work?", value="Simple! Simply use `>modmail [modname]` to get up to {} threads from the most recent {} threads for any mod on the sub. Simply use the arrows to move between threads. Be careful of your spelling, capitalization, and make sure not to include the u/! Be patient, the bot is a bit slow at gathering all the posts.".format(f.threadCap, f.lim), inline=False)
